...
function createTestQuestion(overrides: Partial<GameQuestion> = {}): GameQuestion {
  return {
    id: 'question-1',
    gameType: 'quiz',
...
function createTestGameSession(overrides: Partial<GameSession> = {}): GameSession {
  return {
    id: 'game-session-1',
    sessionId: 'session-1',
    gameType: null,
...
describe('GetGameQuestionsUseCase', () => {
  ...
  it('deve retornar perguntas aleatórias', async () => {
    ...
    expect(mockRepository.findRandom).toHaveBeenCalledWith(2, undefined, undefined);
  });
  ...
  it('deve passar filtros de tipo de jogo e restaurante', async () => {
    vi.mocked(mockRepository.findRandom).mockResolvedValue([createTestQuestion()]);
    ...
    expect(result.success).toBe(true);
    expect(mockRepository.findRandom).toHaveBeenCalledWith(3, ['quiz', 'tinder'], 'rest-1');
  });
...
describe('StartGameSessionUseCase', () => {
  ...
  it('deve criar sessão de jogo e retornar perguntas', async () => {
    ...
    const questions = [createTestQuestion({ id: 'q-1' }), createTestQuestion({ id: 'q-2' })];
  ...
  it('deve calcular round_number baseado em sessões existentes', async () => {
    ...
    vi.mocked(mockSessionRepo.findBySessionId).mockResolvedValue(existingSessions);
  ...
  it('deve usar questionsPerRound personalizado', async () => {
    vi.mocked(mockSessionRepo.findBySessionId).mockResolvedValue([]);
    vi.mocked(mockSessionRepo.create).mockResolvedValue(createTestGameSession());
    vi.mocked(mockQuestionRepo.findRandom).mockResolvedValue([]);
    ...
    expect(mockQuestionRepo.findRandom).toHaveBeenCalledWith(10, undefined, undefined);
  });

  it('deve criar sessão em modo tinder sem buscar perguntas', async () => {
    const tinderSession = createTestGameSession({ gameType: 'tinder', totalQuestions: 0 });

    vi.mocked(mockSessionRepo.findBySessionId).mockResolvedValue([]);
    vi.mocked(mockSessionRepo.create).mockResolvedValue(tinderSession);

    const result = await useCase.execute({ sessionId: 'session-1', gameType: 'tinder' });
...
describe('SubmitGameAnswerUseCase', () => {
  ...
  it('deve calcular pontuação para tinder (like = pontos completos)', async () => {
    const answer = createTestGameAnswer({ gameType: 'tinder', scoreEarned: 10 });
    vi.mocked(mockRepository.create).mockResolvedValue(answer);
  ...
  it('deve calcular pontuação para preference (sempre pontos completos)', async () => {
    const answer = createTestGameAnswer({ gameType: 'preference', scoreEarned: 10 });
    vi.mocked(mockRepository.create).mockResolvedValue(answer);
...
describe('CompleteGameSessionUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new CompleteGameSessionUseCase(mockSessionRepo, mockAnswerRepo, mockPrizeRepo);
  });
  ...
  it('deve completar sessão e retornar leaderboard com prémio', async () => {
    ...
    vi.mocked(mockAnswerRepo.getSessionLeaderboard).mockResolvedValue(leaderboard);
    vi.mocked(mockSessionRepo.findBySessionId).mockResolvedValue(completedSessions);
  ...
  it('não deve atribuir prémio quando tipo é none', async () => {
    vi.mocked(mockSessionRepo.complete).mockResolvedValue(createTestGameSession({ status: 'completed' }));
...
describe('GetGameLeaderboardUseCase', () => {
  ...
  it('deve retornar leaderboard ordenado com rankings', async () => {
    ...
    vi.mocked(mockRepository.getSessionLeaderboard).mockResolvedValue(rawScores);
...
describe('GetGameConfigUseCase', () => {
  ...
  it('deve retornar configuração de jogos do restaurante', async () => {
    ...
    if (result.success) {
      ...
      expect(result.data.gamesPrizeProductId).toBe('prod-1');
...
describe('RedeemGamePrizeUseCase', () => {
  ...
  it('deve retornar erro quando repositório falha', async () => {
    vi.mocked(mockRepository.redeem).mockRejectedValue(new Error('Prize not found'));
