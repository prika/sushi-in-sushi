...
export async function GET(request: NextRequest) {
  ...
  console.info("[Cron] Starting Vendus sync job...");
  ...
  if (!isVendusEnabled()) {
    console.info("[Cron] Vendus not enabled, skipping sync");
  ...
  if (locations.length === 0) {
    console.info("[Cron] No locations configured for Vendus");
    return NextResponse.json({
  ...
  for (const locationSlug of locations) {
    try {
      console.info(`[Cron] Syncing products for ${locationSlug}...`);
  ...
  // Process retry queue
  console.info("[Cron] Processing retry queue...");
  ...
  console.info(`[Cron] Vendus sync completed in ${duration}ms`);
