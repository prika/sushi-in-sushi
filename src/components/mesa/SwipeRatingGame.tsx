...
export function SwipeRatingGame({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#0D0D0D]" style={{ paddingTop: "env(safe-area-inset-top)", paddingBottom: "env(safe-area-inset-bottom)" }}>
      {/* Header + Banner */}
      <div className="shrink-0 px-4 pt-2 pb-3 border-b border-gray-800">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-lg font-semibold text-white">{t("mesa.rate.title")}</h2>
          <button
            type="button"
            onClick={onClose}
          ...
          {leaderProductName && (
            <div className="flex items-center gap-2 rounded-xl bg-[#1A1A1A] border border-[#D4AF37]/30 px-3 py-2">
              ...
              <span className="text-sm text-gray-300">
                {t("mesa.rate.tableLeader")}: <strong className="text-white">{leaderProductName}</strong>
              </span>
            </div>
          )}
          <div className="flex items-center gap-2 rounded-xl bg-[#1A1A1A] border border-gray-700 px-3 py-2">
            ...
            <span className="text-sm text-gray-300">
              {hasWonDrink
                ...
                : sessionCustomerId
                  ? t("mesa.rate.freeDrinkProgress", { count: neededForDrink, min: MIN_RATINGS_FOR_DRINK })
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 relative overflow-hidden">
        {noProducts && (
          <p className="text-gray-500 text-center">{t("mesa.rate.noProducts")}</p>
        )}
        ...
        {finished && (
          ...
          >
            <p className="text-xl font-semibold text-white mb-1">{t("mesa.rate.thanks")}</p>
            <p className="text-gray-400 text-sm mb-2">{t("mesa.rate.thanksSub")}</p>
            ...
            >
              {gameSessionId ? t("mesa.rate.seeLeaderboard") : t("mesa.rate.done")}
            </button>
          </motion.div>
        )}
            ...
            <div className="relative w-full max-w-[320px] aspect-[4/5] flex items-center justify-center">
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  onDrag={(_, info) => setDragDirection(info.offset.x > 0 ? "right" : "left")}
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 text-left">
                    <h3 className="text-xl font-bold text-white drop-shadow">{current.name}</h3>
                    {current.description && (
                      <p className="text-sm text-gray-300 line-clamp-2 mt-0.5">{current.description}</p>
                    )}
                  </div>
